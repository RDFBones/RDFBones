...
public class DataGenerator extends VitroAjaxController {
    ...
    public JSONObject responseFunction(VitroRequest vreq) throws JSONException{
      ...
      EditConfigurationVTwo config = EditConfigurationVTwo.getConfigFromSession(vreq.getSession(), editKey);
      Graph graph = config.getCustomGraph();
      NewURIMaker newUriMaker = new NewURIMakerVitro(vreq.getWebappDaoFactory());
      graph.setNewUriMaker(newUriMaker);
      log.info("Graph");
      graph.debug(0);
      
      Model writeModel = config.getWriteModelSelector().getModel(vreq,getServletContext());  
      String triplesToCreate = graph.saveInitialData(JSON.getDummy1(), vreq);
