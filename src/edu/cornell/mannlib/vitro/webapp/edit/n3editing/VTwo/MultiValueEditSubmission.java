...
public class MultiValueEditSubmission {
  ...
  public Map<String, List<FileItem>> getFilesFromForm() {
    return filesFromForm;
  }

  public void setFilesFromForm(Map<String, List<FileItem>> filesFromForm) {
    this.filesFromForm = filesFromForm;
  }
  ...
  public MultiValueEditSubmission(VitroRequest vreq,
      EditConfigurationVTwo editConfig) {

    this(vreq.getParameterMap(), editConfig);

    /*
     * Map<String, List<FileItem>> map = vreq.getFiles(); List<FileItem> list =
     * map.get(PARAMETER_UPLOADED_FILE); FileItem file = list.get(0);
     * 
     * Map <String,List<FileItem>> files = new HashMap<String,List<FileItem>>();
     * files.put("files", list); this.setFilesFromForm(files);
     * 
     * if (file.getSize() == 0) {
     * 
     * //The request was sent from file upload form
     * if(editConfig.literalsOnForm.contains("dataFile")){ //If it is just an
     * edit if(editConfig.urisInScope.get("fileIndividual") == null ){
     * 
     * //Validation error Map <String,String> fileError = new
     * HashMap<String,String>(); fileError.put("fileError",
     * "Please select a file!"); this.validationErrors.putAll(fileError); } } }
     * else { this.isFile = true; }
     */
  }

  public MultiValueEditSubmission(Map<String, String[]> queryParameters,
      EditConfigurationVTwo editConfig) {
    ...
    if (this.editKey == null || this.editKey.trim().length() == 0)
      throw new Error(
          "EditSubmission needs an 'editKey' parameter from the EditConfiguration");
    ...
    if (editConfig.getValidators() != null) {
      for (N3ValidatorVTwo validator : editConfig.getValidators()) {
        if (validator != null) {
          // throw new
          // Error("need to implemente a validator interface that works with the new MultivalueEditSubmission.");
  ...
  protected void processEditElementFields(EditConfigurationVTwo editConfig,
    ...
    for (String fieldName : editConfig.getFields().keySet()) {
      ...
      if (field != null && field.getEditElement() != null) {
        ...
        log.debug("Checking EditElement for field " + fieldName + " type: "
            + element.getClass().getName());

        // check for validation error messages
        Map<String, String> errMsgs =
            element.getValidationMessages(fieldName, editConfig,
                queryParameters);
        validationErrors.putAll(errMsgs);

        if (errMsgs == null || errMsgs.isEmpty()) {
          // only check for uris and literals when element has no validation
          // errors
          Map<String, List<String>> urisFromElement =
              element.getURIs(fieldName, editConfig, queryParameters);
          if (urisFromElement != null)
            urisFromForm.putAll(urisFromElement);
          Map<String, List<Literal>> literalsFromElement =
              element.getLiterals(fieldName, editConfig, queryParameters);
          if (literalsFromElement != null)
            literalsFromForm.putAll(literalsFromElement);
        } else {
          log.debug("got validation errors for field " + fieldName
              + " not processing field for literals or URIs");
        }
      }
    }
  }
  ...
  public Literal createLiteral(String value, String datatypeUri, String lang) {
    if (datatypeUri != null) {
      if ("http://www.w3.org/2001/XMLSchema:anyURI".equals(datatypeUri)) {
        try {
          return literalCreationModel.createTypedLiteral(
              URLEncoder.encode(value, "UTF8"), datatypeUri);
  ...
  private static DateTimeFormatter dformater = DateTimeFormat
      .forPattern("yyyy-MM-dd'T'HH:mm:00");
  private static DateTimeFormatter dateFormater = DateTimeFormat
   ...
   */
  public Literal getTime(Map<String, String[]> queryParameters,
    String fieldName) {
    List<String> hour = Arrays.asList(queryParameters.get("hour" + fieldName));
    List<String> minute =
        Arrays.asList(queryParameters.get("minute" + fieldName));

    if (hour == null || hour.size() == 0 || minute == null
        || minute.size() == 0) {
      log.info("Could not find query parameter values for time field "
          + fieldName);
    ...
    String hourStr =
        (hourInt < 10) ? "0" + Integer.toString(hourInt) : Integer
            .toString(hourInt);
    String minuteStr =
        (minuteInt < 10) ? "0" + Integer.toString(minuteInt) : Integer
            .toString(minuteInt);
    String secondStr = "00";

    return new EditLiteral(hourStr + ":" + minuteStr + ":" + secondStr,
        TIME_URI, null);
  ...
  public void addLiteralToForm(EditConfigurationVTwo editConfig,
    FieldVTwo field, String var, String[] valuesArray) {
    List<String> valueList =
        (valuesArray != null) ? Arrays.asList(valuesArray) : null;
    if (valueList != null && valueList.size() > 0) {
      ...
      for (String value : valueList) {
        ...
        if (!StringUtils.isEmpty(value)) {
          literalsArray.add(createLiteral(value, field.getRangeDatatypeUri(),
              field.getRangeLang()));
        }
      }
  ...
  // Add literal to form
  // Add uri to form
  public void addUriToForm(EditConfigurationVTwo editConfig, String var,
    String[] valuesArray) {
    List<String> values =
        (valuesArray != null) ? Arrays.asList(valuesArray) : null;
    if (values != null && values.size() > 0) {
      // Iterate through the values and check to see if they should be added or
      // removed from form
      urisFromForm.put(var, values);
      for (String uri : values) {
        if (uri != null && uri.length() == 0
            && editConfig.getNewResources().containsKey(var)) {
          log.debug("A new resource URI will be made for var " + var
              + " since it was blank on the form.");
  ...
  public boolean hasLiteralValue(String key) {
    return (this.literalsFromForm.containsKey(key) && this.literalsFromForm
        .get(key) != null);
  }
